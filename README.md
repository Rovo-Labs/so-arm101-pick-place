# gpudad_7n_beta_world_v1 -- static world

A frozen MuJoCo scene. No Python, no build step: open a scene file in any
MuJoCo viewer.
...
MAE is mean absolute pixel error, 0-255 per channel, against real GPU-DAD frames. Lower is closer. Regenerate with `verify_static_world.py`.

Generated by `export_static_world.py` -- edit that, not these files.
